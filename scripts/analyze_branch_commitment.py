#!/usr/bin/env python3
"""Analyze developmental branch commitment in AVA event latents."""
...
from ava.analysis.branch_commitment import (  # noqa: E402
	DEFAULT_BRANCH_BEAD,
	run_branch_commitment_analysis,
)


def _default_run_name() -> str:
	return time.strftime("%Y%m%d-%H%M%S-branch-commitment", time.localtime())


def main() -> None:
	parser = argparse.ArgumentParser(
		description="Post-hoc developmental branch-commitment analysis for AVA event latents."
	)
	parser.add_argument("--event-table", type=Path, default=None)
	parser.add_argument("--coverage-metrics", type=Path, default=None)
	parser.add_argument(
		"--manifest",
		type=Path,
	...
	parser.add_argument("--bead-id", type=str, default=DEFAULT_BRANCH_BEAD)
	...
	if args.event_table is None and args.latent_root is None:
		raise ValueError("Provide --event-table, or provide --latent-root for full rebuild mode.")
	...
	metrics = run_branch_commitment_analysis(
