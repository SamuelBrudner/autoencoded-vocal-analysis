...
def clean_segments_by_hand(
    ...
    * This will not overwrite existing segmentation files and will raise an
    ...
    temp_dict = dict(zip(orig_seg_dirs, new_seg_dirs, strict=False))
    new_seg_fns = []
    ...
    for new_seg_fn in new_seg_fns:
        assert not os.path.isfile(new_seg_fn), "File already exists: " + new_seg_fn
    # Collect all of the segments.
    all_onsets, all_offsets = [], []
    all_audio_fns, all_orig_seg_fns, all_new_seg_fns = [], [], []
    gen = zip(audio_fns, orig_seg_fns, new_seg_fns, strict=False)
...
def copy_segments_to_standard_format(
    orig_seg_dirs, new_seg_dirs, seg_ext, delimiter, usecols, skiprows, max_duration=None
):
    """
    ...
    for orig_seg_dir, new_seg_dir in zip(orig_seg_dirs, new_seg_dirs, strict=False):
        if not os.path.exists(new_seg_dir):
            os.makedirs(new_seg_dir)
        seg_fns = [
            os.path.join(orig_seg_dir, i)
            for i in os.listdir(orig_seg_dir)
            if len(i) > 4 and i[-4:] == seg_ext
        ]
        for seg_fn in seg_fns:
            segs = np.loadtxt(
                seg_fn, delimiter=delimiter, skiprows=skiprows, usecols=usecols
            ).reshape(-1, 2)
...
def write_segments_to_audio(in_audio_dirs, out_audio_dirs, seg_dirs, n_zfill=3, verbose=True):
    """
    ...
    verbose : bool, optional
            Deafults to ``True``.
    """
    assert len(in_audio_dirs) == len(
        out_audio_dirs
    ), f"{len(in_audio_dirs)} != {len(out_audio_dirs)}"
    assert len(in_audio_dirs) == len(seg_dirs), f"{len(in_audio_dirs)} != {len(seg_dirs)}"
    if verbose:
        print("Writing segments to audio,", len(in_audio_dirs), "directories")
    for in_dir, out_dir, seg_dir in zip(in_audio_dirs, out_audio_dirs, seg_dirs, strict=False):
        ...
        if not os.path.exists(out_dir):
            os.makedirs(out_dir)
        for seg_fn, audio_fn in zip(seg_fns, audio_fns, strict=False):
...
def merge_segments(
    ...
    for orig_seg_dir, new_seg_dir in zip(orig_seg_dirs, new_seg_dirs, strict=False):
        orig_seg_fns = [
            os.path.join(orig_seg_dir, i) for i in os.listdir(orig_seg_dir) if _is_txt_file(i)
        ]
        new_seg_fns = [os.path.join(new_seg_dir, os.path.split(i)[1]) for i in orig_seg_fns]
        for orig_seg_fn, new_seg_fn in zip(orig_seg_fns, new_seg_fns, strict=False):
...
def get_audio_seg_filenames(audio_dirs, seg_dirs):
    ...
    """
    assert len(audio_dirs) == len(seg_dirs), f"{len(audio_dirs)} != {len(seg_dirs)}"
    audio_fns, seg_fns = [], []
    for audio_dir, seg_dir in zip(audio_dirs, seg_dirs, strict=False):
        temp_fns = [i for i in sorted(os.listdir(audio_dir)) if _is_audio_file(i)]
