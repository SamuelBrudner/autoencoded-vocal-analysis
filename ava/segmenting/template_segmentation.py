...
def get_template(feature_dir, p, smoothing_kernel=(0.5, 0.5), verbose=True):
    ...
    """
    filenames = [os.path.join(feature_dir, i) for i in os.listdir(feature_dir) if _is_wav_file(i)]
    specs = []
...
def segment_files(
    audio_dirs, segment_dirs, template, p, num_mad=2.0, min_dt=0.05, n_jobs=1, verbose=True
):
    """
    ...
    all_audio_fns = []
    all_seg_dirs = []
    for audio_dir, segment_dir in zip(audio_dirs, segment_dirs, strict=False):
        if not os.path.exists(segment_dir):
            os.makedirs(segment_dir)
        audio_fns = [os.path.join(audio_dir, i) for i in os.listdir(audio_dir) if _is_wav_file(i)]
    ...
    gen = zip(
        all_seg_dirs, all_audio_fns, repeat(template), repeat(p), repeat(num_mad), repeat(min_dt)
    )
...
def _segment_file(
    segment_dir, filename, template, p, num_mad=2.0, min_dt=0.05, min_extra_time_bins=5
):
    """
...
def clean_collected_segments(
    ...
    if len(specs) > max_num_specs:
        warnings.warn(
            ...
            + str(max_num_specs)
            + "). Consider increasing `max_num_specs` or"
            + " `num_mad`.",
            UserWarning,
        )
    if verbose:
    ...
    while True:
        ...
        if i == 0:
            ...
            tooltip_plot(
                embedding, specs, output_dir=tooltip_plot_dir, num_imgs=1000, title=title, grid=True
            )
            if verbose:
                print("\tDone.")
        # Get input from user.
        for key, query in zip(bounds_keys, queries, strict=False):
    ...
    for audio_dir, seg_dir in zip(audio_dirs, segment_dirs, strict=False):
        audio_fns = [os.path.join(audio_dir, i) for i in os.listdir(audio_dir) if _is_wav_file(i)]
        for audio_fn in audio_fns:
            with warnings.catch_warnings():
                warnings.filterwarnings("ignore", category=WavFileWarning)
...
def segment_sylls_from_songs(
    audio_dirs, song_seg_dirs, syll_seg_dirs, p, shoulder=0.05, img_fn="temp.pdf", verbose=True
):
    """
    ...
    * All the song segments must be the same duration!
    ...
    for audio_fn in song_segs:
        ...
        for seg in song_segs[audio_fn].reshape(-1, 2):
            ...
            if i1 < 0 or i2 > len(audio):
                ...
                new_spec = np.mean(spec) * np.ones(
                    (spec.shape[0], spec.shape[1] + pre_bins + post_bins)
                )
                if post_bins == 0:
    ...
    while True:
        ...
        axarr[0].imshow(
            specs[np.random.randint(len(specs))],
            origin="lower",
            aspect="auto",
        ...
        for q in quantiles:
            for i in range(len(temp)):
                try:
                    ...
                    pass
        axarr[1].imshow(temp, origin="lower", aspect="auto", extent=[0, max_t, 0, len(amp_traces)])
        temp = np.copy(aligned)
        for q in quantiles:
            for i in range(len(temp)):
                temp[i, int(round(q * num_time_bins))] = max_aligned_val
        axarr[2].imshow(temp, origin="lower", aspect="auto", extent=[0, max_t, 0, len(amp_traces)])
    ...
    quantiles.sort()
    files_encountered = {}
    for i, (fn, song_onset) in enumerate(zip(fns, song_onsets, strict=False)):
...
def segment_sylls_from_warped_songs(
    warped_window_dset,
    audio_dirs,
    spec_dirs,
    time_bins=512,
    ...
    """
    # Check the input.
    audio_dir_to_spec_dir = dict(zip(audio_dirs, spec_dirs, strict=False))
    for audio_fn in warped_window_dset.audio_filenames:
        assert os.path.split(audio_fn)[0] in audio_dirs, (
            "Cannot find " + os.path.split(audio_fn)[0] + " in audio_dirs!"
        )
    ...
    segs = [[q1, q2] for q1, q2 in zip(quantiles[:-1], quantiles[1:], strict=False)]
    num_saved = 0
    ...
    for audio_fn in warped_window_dset.audio_filenames:
        ...
        for q1, q2 in segs:
            ...
            spec, flag = p["get_spec"](
                0.0,
                template_dur,
                warped_window_dset.audio[index],
                p,
                fs=fs,
                max_dur=None,
        ...
        with h5py.File(write_fn, "w") as f:
            ...
            f.create_dataset(
                "audio_filenames", data=np.array(syll_data["audio_filenames"]).astype("S")
            )
    if verbose:
