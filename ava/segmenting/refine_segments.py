...
from ava.segmenting.utils import _read_onsets_offsets, get_audio_seg_filenames, get_spec


def refine_segments_pre_vae(
    seg_dirs,
    audio_dirs,
    out_seg_dirs,
    p,
    ...
    while True:
        ...
        if (
            input(
                "Press [q] to quit identifying noise or \
				[return] to continue: "
            )
            == "q"
        ):
...
def refine_segments_post_vae(
    ...
    for audio_dir, out_seg_dir in zip(audio_dirs, out_seg_dirs, strict=False):
...
def _get_specs(audio_dirs, seg_dirs, p, max_num_specs=None, max_len=None, return_segs=False):
    """
    ...
    for audio_fn, seg_fn in zip(audio_fns, seg_fns, strict=False):
        ...
        assert len(audio) >= p["nperseg"], (
            "Short audio file: " + audio_fn + ", duration: " + str(len(audio) / fs)
        )
...
def _update_segs_helper(seg_dir, audio_dir, out_seg_dir, p, max_len, transform, bounds, verbose):
    """
    ...
    specs, _, all_fns = _get_specs([audio_dir], [seg_dir], p, max_len=max_len)
    specs = np.stack(specs)
...
def _in_bounds(point, bounds):
    ...
    for i in range(len(bounds["x1"])):
        if (
            point[0] > bounds["x1"][i]
            and point[0] < bounds["x2"][i]
            and point[1] > bounds["y1"][i]
            and point[1] < bounds["y2"][i]
        ):
            return True
    return False
