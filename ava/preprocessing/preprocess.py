...
from ava.preprocessing.utils import _inv_mel, _mel

EPSILON = 1e-12


def process_sylls(audio_dir, segment_dir, save_dir, p, shuffle=True, verbose=True):
    """
    ...
    for audio_filename, seg_filename in zip(audio_filenames, seg_filenames, strict=False):
        ...
        while len(syll_data["onsets"]) >= sylls_per_file:
            ...
            with h5py.File(save_filename, "w") as f:
                ...
                for key in ["onsets", "offsets"]:
                    f.create_dataset(key, data=np.array(syll_data[key][:sylls_per_file]))
                f.create_dataset("specs", data=np.stack(syll_data["specs"][:sylls_per_file]))
                temp = [
                    os.path.join(audio_dir, i)
                    for i in syll_data["audio_filenames"][:sylls_per_file]
                ]
            ...
            if (
                p["max_num_syllables"] is not None
                and write_file_num * sylls_per_file >= p["max_num_syllables"]
            ):
                if verbose:
                    print(
                        "\tSaved max_num_syllables ("
                        + str(p["max_num_syllables"])
                        + "). Returning."
                    )
                return
    if verbose:
...
def get_syll_specs(onsets, offsets, audio_filename, p):
    ...
    # For each syllable...
    for i, t1, t2 in zip(range(len(onsets)), onsets, offsets, strict=False):
        spec, valid = p["get_spec"](t1, t2, audio, p, fs, target_freqs=target_freqs)
        if valid:
    ...
    while True:
        ...
        while temp != "s" and temp != "r":
            ...
            spec, flag = p["get_spec"](
                0.0, duration, audio, p, fs=p["fs"], max_dur=None, target_times=target_times
            )
            assert flag
...
def get_audio_filenames(audio_dir):
    ...
    fns = [os.path.join(audio_dir, i) for i in sorted(os.listdir(audio_dir)) if is_audio_file(i)]
    return fns
