    ...
    for warp_iter in range(total_iterations):
        ...
        for i in range(len(specs)):
            ...
            if slope_λs[warp_iter] == np.inf:
                objective = _get_shift_objective(specs[i], mean_spec, interps[i], shift_λ)
            else:
                objective = _get_linear_objective(specs[i], mean_spec, interps[i], shift_λ, slope_λ)
            # Optimize.
            ...
            loss = res.fun
            # Update warped specs.
            warped_specs[i] = interps[i](x0[i, 0] + np.exp(x0[i, 1]) * np.arange(specs[i].shape[1]))
        if verbose:
            temp_loss = round(np.mean(loss), 3)
            print(f"Iteration {warp_iter}, loss={temp_loss}")
