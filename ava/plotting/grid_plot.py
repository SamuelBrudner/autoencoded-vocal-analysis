...
def indexed_grid_plot_DC(
    dc, indices, ax=None, save_and_close=True, gap=3, side_len=128, filename="grid.pdf"
):
    """
    Plot a grid of spectrograms.
    ...
    grid_plot(result, gap=gap, ax=ax, save_and_close=save_and_close, filename=filename)


def grid_plot(specs, gap=3, vmin=0.0, vmax=1.0, ax=None, save_and_close=True, filename="temp.pdf"):
    """
    Parameters
    ----------
    specs : numpy.ndarray
    ...
    if ax is None:
        ax = plt.gca()
    ax.imshow(img, aspect="equal", origin="lower", interpolation="none", vmin=vmin, vmax=vmax)
    ax.axis("off")
    if save_and_close:
        plt.tight_layout()
