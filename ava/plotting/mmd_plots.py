...
def mmd_matrix_plot(
    ...
    if colorbar:
        fig = plt.gcf()
        cbar = fig.colorbar(im, cax=cax, fraction=0.046, orientation="horizontal", ticks=ticks)
        cbar.solids.set_edgecolor("face")
        cbar.solids.set_rasterized(True)
        labels = [f"{round(tick, 1):.1f}" for tick in ticks]
...
def mmd_tsne_plot_DC(
    ...
    [COLOR_LIST[i % len(COLOR_LIST)] for i in all_conditions]
    transform = TSNE(
        n_components=2, random_state=42, metric="precomputed", method="exact", perplexity=perplexity
    )
    ...
    for i in range(len(conditions) - 1):
        for j in range(i + 1, len(conditions)):
            if conditions[i] == conditions[j]:
                ...
                ax.plot([embed[i, 0], embed[j, 0]], [embed[i, 1], embed[j, 1]], c=color, lw=0.5)
...
def _estimate_mmd2_linear_time(latent, i1, i2, sigma=None):
    ...
    def k(x, y):
        return np.exp(A * np.sum(np.power(x - y, 2)))
    ...
    for i in range(m):
        term += h(
            latent[i1[2 * i]], latent[i2[2 * i]], latent[i1[2 * i + 1]], latent[i2[2 * i + 1]]
        )
    return term / m
...
def _calculate_mmd2(
    ...
    if parallel:
        ...
        n_jobs = os.cpu_count()
        # Calculate.
        temp_results = Parallel(n_jobs=n_jobs)(delayed(_mmd2_helper)(*args) for args in gen)
        ...
        for i in range(n - 1):
            for j in range(i + 1, n):
                ...
                if alg == "linear":
                    temp = _estimate_mmd2_linear_time(latent, i1, i2, sigma=sigma)
                elif alg == "quadratic":
                    temp = _estimate_mmd2(latent, i1, i2, sigma=sigma, max_n=max_n)
                else:
...
def _mmd2_helper(i, j, condition, all_conditions, alg, latent, sigma, max_n):
...
def _matrix_from_txt(text_fn):
    ...
    for i, j, mmd2 in zip(i_s, j_s, mmd2s, strict=False):
