...
def shotgun_movie_DC(
    dc,
    audio_file,
    p,
    method="spectrogram_correlation",
    output_dir="temp",
    fps=30,
    shoulder=0.01,
    c="b",
    ...
    while onset + p["window_length"] < len(audio) / fs - shoulder:
        ...
        spec, flag = p["get_spec"](
            onset - shoulder, offset + shoulder, audio, p, fs=fs, target_times=target_times
        )
        assert flag
        specs.append(spec)
        onset += dt
    ...
    for i in range(len(new_embed)):
        plt.scatter(
            original_embed[:, 0],
            original_embed[:, 1],
            c=[c] * len(original_embed),
            alpha=alpha,
            s=s,
        )
        plt.scatter(
            [new_embed[i, 0]], [new_embed[i, 1]], s=marker_s, marker=marker_marker, c=marker_c
        )
        plt.xlim(xmin, xmax)
        plt.ylim(ymin, ymax)
    ...
    os.path.join(output_dir, mp4_fn)
    bashCommand = "ffmpeg -y -r {} -i {} {}".format(fps, img_fns, "temp.mp4")
    process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
    output, error = process.communicate()
    bashCommand = "ffmpeg -y -r {} -i {} -i {} -c:a aac -strict " + f"-2 {fps}"
    process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
    output, error = process.communicate()
