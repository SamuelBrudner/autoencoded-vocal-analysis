...
def latent_projection_plot_DC(
    dc,
    embedding_type="latent_mean_umap",
    color_by=None,
    title=None,
    filename="latent.pdf",
    colorbar=False,
...
def latent_projection_plot_with_noise_DC(
    dc,
    noise_box,
    embedding_type="latent_mean_umap",
    color_by=None,
...
def projection_plot(
    ...
    if colorbar:
        ...
        ticks = [
            int(round(i))
            for i in [
                0.8 * min_val + 0.2 * max_val,
                0.5 * (min_val + max_val),
                0.8 * max_val + 0.2 * min_val,
            ]
        ]
        fig = plt.gcf()
        if cax is None:
