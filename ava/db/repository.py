...
from ava.db.schema import Annotation, Embedding, Recording, Syllable
...
class RecordingRepository:
    ...
    def create(
        self, file_path: str, checksum_sha256: str, metadata: dict[str, str] | None = None
    ) -> Recording:
        """Create new recording with integrity validation."""
        recording = Recording(
            file_path=file_path, checksum_sha256=checksum_sha256, extra_metadata=metadata
        )
...
class SyllableRepository:
    ...
    def get_with_annotations(self, syllable_id: int) -> Syllable | None:
        """Retrieve syllable with eagerly loaded annotations relationship."""
        query = select(Syllable).where(Syllable.id == syllable_id)
        result = self.session.execute(query).scalar_one_or_none()
        return result
...
class AnnotationRepository:
    ...
    def filter_by_type_and_key(self, annotation_type: str, key: str) -> list[Annotation]:
        ...
        query = (
            select(Annotation)
            .where(Annotation.annotation_type == annotation_type, Annotation.key == key)
...
class QueryDSL:
    ...
    def filter_by_label(self, annotation_type: str, label_value: str) -> "QueryDSL":
        ...
        self._query = self._query.where(
            Annotation.annotation_type == annotation_type, Annotation.value == label_value
        )
        return self
