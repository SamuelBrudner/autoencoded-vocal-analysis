...
class Syllable(Base):
    ...
    recording_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("recording.id", ondelete="CASCADE"), nullable=False
    )
    spectrogram_path: Mapped[str] = mapped_column(String, nullable=False)
    start_time: Mapped[float] = mapped_column(Float, nullable=False)
    end_time: Mapped[float] = mapped_column(Float, nullable=False)
    bounds_metadata: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
    ...
    annotations = relationship(
        "Annotation", back_populates="syllable", cascade="all, delete-orphan"
    )
...
class Embedding(Base):
    ...
    syllable_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("syllable.id", ondelete="CASCADE"), nullable=False
    )
    model_version: Mapped[str] = mapped_column(String, nullable=False)
    embedding_path: Mapped[str] = mapped_column(String, nullable=False)
    dimensions: Mapped[int] = mapped_column(Integer, nullable=False)
    extra_model_metadata: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
