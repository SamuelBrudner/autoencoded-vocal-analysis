...
class DataContainer:
    ...
    def _read_segment_audio(self):
        ...
        for audio_dir in self.audio_dirs:
            dir_result = {}
            audio_fns = [
                i for i in os.listdir(audio_dir) if _is_wav_file(i) and i in segments[audio_dir]
            ]
    ...
    def _make_sap_time(self):
        ...
        result = np.zeros(len(onsets))
        for i, onset, fn in zip(range(len(onsets)), onsets, fns, strict=False):
