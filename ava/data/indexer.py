...
from ava.db.repository import RecordingRepository, SyllableRepository
...
class FilesystemIndexer:
    ...
    def extract_hdf5_metadata(self, hdf5_path: Path) -> dict[str, Any]:
        ...
        try:
            with h5py.File(resolved_path, "r") as h5_file:
                ...
                if missing_keys:
                    raise RuntimeError(
                        f"Missing required datasets {missing_keys} in {resolved_path}"
                    )
            ...
            raise RuntimeError(f"Failed to extract metadata from {resolved_path}: {e}") from e
    ...
    def compute_checksums(self, file_paths: list[Path]) -> dict[Path, str]:
        ...
        for file_path in file_paths:
            ...
            try:
                ...
                raise RuntimeError(f"Checksum computation failed for {file_path}: {e}") from e
                ...
                try:
                    ...
                    if existing_recording is None:
                        recording = recording_repo.create(
                            ...
                            metadata={
                                "file_type": "hdf5_spectrogram",
                                "num_syllables": metadata["num_syllables"],
                            },
                        )
                    ...
                    for i, (onset, offset) in enumerate(
                        zip(metadata["onsets"], metadata["offsets"], strict=False)
                    ):
                        syllable_data = {
                            ...
                            "bounds_metadata": {
                                "batch_index": i,
                                "specs_shape": metadata["specs_shape"],
                            },
                        }
                    ...
                    raise RuntimeError(f"Database population failed for {hdf5_path}: {e}") from e
    ...
    def validate_integrity(self, file_paths: list[Path]) -> bool:
        ...
        for file_path in file_paths:
            ...
            elif file_path.suffix.lower() not in [".h5", ".hdf5"]:
                validation_errors.append(f"Invalid file extension: {file_path}")

        if validation_errors:
            error_msg = "Integrity validation failed:\n" + "\n".join(validation_errors)
            raise RuntimeError(error_msg)

        logger.info(f"Integrity validation passed for {len(file_paths)} files")
        return True

    def run_full_indexing(self, base_directory: Path | None = None) -> dict[str, Any]:
        ...
        logger.info("Starting filesystem indexing", base_dir=str(base_directory))
        ...
        try:
            ...
            results = {
                ...
                "total_syllables": sum(
                    len(self.extract_hdf5_metadata(f)["onsets"]) for f in discovered_files
                ),
    ...
    def from_config_dict(cls, config_dict: dict[str, Any], engine: Engine) -> "FilesystemIndexer":
        ...
        try:
            ...
            logger.debug(
                "Configuration validation successful", config=validated_config.model_dump()
            )
