...
def cross_validation_warp_parameter_search(
    audio_dirs,
    spec_params,
    search_params=None,
    ...
    * All `.wav` files should be the same duration!
    ...
    specs, amps, _ = _get_specs_and_amplitude_traces(all_audio, fs, spec_params)
    if verbose:
        print("\tDone.")
...
def anchor_point_warp_parameter_search(
    audio_dirs,
    anchor_dir,
    spec_params,
    search_params,
    ...
    * All `.wav` files should be the same duration!
    ...
    if verbose:
        print("Null warp MAE:", f"{null_warp_mae:.3f}", "ms")
    for i in range(1, len(anchor_times)):
        assert len(anchor_times[0]) == len(anchor_times[i]), "Unequal numbers" + " of anchor times!"
    # Collect audio filenames.
    if verbose:
        print("Collecting spectrograms...")
    audio_fns = []
    ...
    with warnings.catch_warnings():
        ...
        all_audio = [wavfile.read(audio_fn)[1] for audio_fn in audio_fns]
        fs = wavfile.read(audio_fns[0])[0]
    specs, amps, template_dur = _get_specs_and_amplitude_traces(all_audio, fs, spec_params)
    if warp_type == "amplitude":
        to_warp = amps
    else:
        to_warp = specs
    ...
    eval_func = _get_eval_func(
        support, anchor_times, to_warp, aw_iterations, aw_warp_iterations, template_dur
    )
    ...
    for i in range(num_iter):  # num_iter
        ...
        if verbose:
            print("\t" + str(param_history[i]), f"{loss:.3f}", "ms")
        loss_history[i] = loss
    # Plot objective vs. parameter marginals.
    if make_plot:
        _, axarr = plt.subplots(nrows=3)
        for i, (ax, key) in enumerate(zip(axarr, PARAM_NAMES, strict=False)):
            ...
            ax.spines["right"].set_visible(False)
            plt.sca(ax)
            plt.xticks(param_ranges[i], [f"{j:.5f}" for j in support[i]])
        plt.tight_layout()
        plt.savefig(img_fn)
        plt.close("all")
...
def _get_eval_func(support, anchor_times, to_warp, aw_iterations, aw_warp_iterations, template_dur):
    ...
    def eval_func(params):
        ...
        model.fit(
            to_warp, iterations=aw_iterations, warp_iterations=aw_warp_iterations, verbose=False
        )
        # Evaluate.
        ...
        for i in range(len(anchor_times)):
            ...
            interp = interp1d(
                x_knots, y_knots, bounds_error=False, fill_value="extrapolate", assume_sorted=True
            )
...
def get_hdf5s_from_dir(dir):
    ...
    """
    return [os.path.join(dir, f) for f in sorted(os.listdir(dir)) if _is_hdf5_file(f)]


def _get_wavs_from_dir(dir):
    """Return a sorted list of wave files from a directory."""
    return [os.path.join(dir, f) for f in sorted(os.listdir(dir)) if _is_wav_file(f)]


def _get_txts_from_dir(dir):
    """Return a sorted list of text files from a directory."""
    return [os.path.join(dir, f) for f in sorted(os.listdir(dir)) if _is_txt_file(f)]
