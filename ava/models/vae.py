...
class VAE(nn.Module):
    r"""Variational Autoencoder class for single-channel images.
    ...
    def train_epoch(self, train_loader):
        ...
        print(f"Epoch: {self.epoch} Average loss: {train_loss:.4f}")
        self.epoch += 1
        return train_loss
    ...
    def test_epoch(self, test_loader):
        ...
        test_loss = 0.0
        with torch.no_grad():
            for _i, data in enumerate(test_loader):
                data = data.to(self.device)
                loss = self.forward(data)
                test_loss += loss.item()
    ...
    def train_loop(self, loaders, epochs=100, test_freq=2, save_freq=10, vis_freq=1):
        """
    ...
    def visualize(self, loader, num_specs=5, gap=(2, 6), save_filename="reconstruction.pdf"):
        ...
        indices = np.random.choice(np.arange(len(loader.dataset)), size=num_specs, replace=False)
