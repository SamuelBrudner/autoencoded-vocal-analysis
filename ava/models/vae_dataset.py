...
from ava.models.utils import _get_sylls_per_file, get_hdf5s_from_dir, numpy_to_tensor
...
def get_syllable_data_loaders(partition, batch_size=64, shuffle=(True, False), num_workers=4):
    """
    ...
    train_dataset = SyllableDataset(
        filenames=partition["train"], transform=numpy_to_tensor, sylls_per_file=sylls_per_file
    )
    ...
    test_dataset = SyllableDataset(
        filenames=partition["test"], transform=numpy_to_tensor, sylls_per_file=sylls_per_file
    )
