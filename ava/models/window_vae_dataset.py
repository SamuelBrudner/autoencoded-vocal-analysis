...
from ava.models.utils import _get_specs_and_amplitude_traces, _get_wavs_from_dir, numpy_to_tensor
...
def get_window_partition(
    audio_dirs, roi_dirs, split=0.8, shuffle=True, exclude_empty_roi_files=True
):
    """
    ...
    for audio_dir, roi_dir in zip(audio_dirs, roi_dirs, strict=False):
    ...
    return {
        "train": {"audio": audio_filenames[:i], "rois": roi_filenames[:i]},
        "test": {"audio": audio_filenames[i:], "rois": roi_filenames[i:]},
    }


def get_fixed_window_data_loaders(
    partition, p, batch_size=64, shuffle=(True, False), num_workers=4, min_spec_val=None
):
    """
    ...
    train_dataloader = DataLoader(
        train_dataset, batch_size=batch_size, shuffle=shuffle[0], num_workers=num_workers
    )
    if not partition["test"]:
        return {"train": train_dataloader, "test": None}
    test_dataset = FixedWindowDataset(
        partition["test"]["audio"],
        partition["test"]["rois"],
        p,
        transform=numpy_to_tensor,
        min_spec_val=min_spec_val,
    ...
    def __getitem__(self, index, seed=None, shoulder=0.05, return_seg_info=False):
        ...
        for _i in index:
            while True:
                ...
                self.filenames[file_index]
                roi_index = np.random.choice(
                    np.arange(len(self.roi_weights[file_index])), p=self.roi_weights[file_index]
                )
    ...
    def write_hdf5_files(self, save_dir, num_files=500, sylls_per_file=100):
        ...
        * This should be consistent with
        ...
        for write_file_num in range(num_files):
            specs, file_indices, _, _ = self.__getitem__(
                np.arange(sylls_per_file), seed=write_file_num, return_seg_info=True
            )
...
def get_warped_window_data_loaders(
    audio_dirs,
    p,
    batch_size=64,
    num_workers=4,
...
class WarpedWindowDataset(Dataset):
    ...
    def __init__(
        ...
        * Use `affinewarp` functions instead of direct references to knots.
        ...
        self._compute_warp(load_warp=load_warp, save_warp=save_warp, warp_type=warp_type)
    ...
    def write_hdf5_files(self, save_dir, num_files=400, sylls_per_file=100):
        ...
        * Add the option to also write segments. This could be useful for noise
        ...
        if save_dir != "" and not os.path.exists(save_dir):
            os.mkdir(save_dir)
        for write_file_num in range(num_files):
            specs = self.__getitem__(np.arange(sylls_per_file), seed=write_file_num)
            specs = np.array([spec.detach().numpy() for spec in specs])
    ...
    def _get_unwarped_times(self, y_vals, index):
        ...
        interp = interp1d(
            y_knots, x_knots, bounds_error=False, fill_value="extrapolate", assume_sorted=True
        )
        x_vals = interp(y_vals)
        return x_vals

    def _compute_warp(self, load_warp=False, save_warp=True, warp_type="spectrogram"):
        """
        ...
        """
        if save_warp:
            assert self.warp_fn is not None, "``warp_fn`` must be specified " + "to save warps!"
        ...
        if warp_type == "null":
            ...
            _, _, template_dur = _get_specs_and_amplitude_traces(self.audio, self.fs, self.p)
        ...
        if load_warp:
            if self.warp_fn is None:
                warnings.warn("Tried to load warps, but ``warp_fns`` is None.", UserWarning)
            else:
                try:
                    ...
                    assert np.all(temp_fns[:-1] <= temp_fns[1:]), (
                        "Filenames " + "in " + self.warp_fn + " are not sorted!"
                    )
                    ...
                    warnings.warn("Can't load warps from: " + str(self.warp_fn), UserWarning)
        # Otherwise, first make the spectrograms.
        amps, specs, template_dur = _get_specs_and_amplitude_traces(self.audio, self.fs, self.p)
        self.template_dur = template_dur
        # Then warp.
    ...
    def get_specific_item(self, query_filename, quantile):
        ...
        start_t = self.start_q + quantile * (self.stop_q - self.start_q - self.window_frac)
        stop_t = start_t + self.window_frac
        t_vals = np.linspace(start_t, stop_t, self.p["num_time_bins"])
        # Inverse warp.
        target_ts = self._get_unwarped_times(t_vals, file_index)
