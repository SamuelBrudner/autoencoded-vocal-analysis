...
from ava.db.repository import EmbeddingRepository, QueryDSL, SyllableRepository
from ava.db.schema import Annotation, Base, Embedding, Recording, Syllable
from ava.db.session import create_engine_from_url, get_session
...
def create_test_data_fixtures(setup_test_database: Engine) -> Generator[Engine, Any, None]:
    ...
    with get_session(engine) as session:
        ...
        syllable1 = Syllable(
            ...
            spectrogram_path="/test/spec1.h5",
            start_time=0.0,
            end_time=0.1,
        )  # 0.1s duration
        syllable2 = Syllable(
            recording_id=recording1.id,
            spectrogram_path="/test/spec2.h5",
            start_time=1.0,
            end_time=1.2,
        )  # 0.2s duration
        syllable3 = Syllable(
            recording_id=recording2.id,
            spectrogram_path="/test/spec3.h5",
            start_time=2.0,
            end_time=2.05,
        )  # 0.05s duration
        ...
        embedding1 = Embedding(
            syllable_id=syllable1.id,
            model_version="vae_v1",
            embedding_path="/test/emb1.npy",
            dimensions=64,
        )
        embedding2 = Embedding(
            syllable_id=syllable2.id,
            model_version="vae_v2",
            embedding_path="/test/emb2.npy",
            dimensions=128,
        )
        ...
        annotation1 = Annotation(
            syllable_id=syllable1.id, annotation_type="label", key="call_type", value="call_A"
        )
        annotation2 = Annotation(
            syllable_id=syllable2.id, annotation_type="label", key="call_type", value="call_B"
        )
