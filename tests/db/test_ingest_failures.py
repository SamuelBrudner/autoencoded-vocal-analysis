...
def test_extract_metadata_missing_file():
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        with pytest.raises(RuntimeError, match=f"HDF5 file not found or not a file: {missing_file.resolve()}"):
...
def test_extract_metadata_wrong_hdf5_keys():
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        with pytest.raises(RuntimeError, match="Missing required datasets.*specs.*onsets.*offsets.*audio_filenames"):
...
def test_checksum_computation_missing_file():
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        with pytest.raises(RuntimeError, match=f"Cannot compute checksum - file not found: {missing_file}"):
...
def test_hdf5_corruption_during_extraction():
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        with pytest.raises(RuntimeError, match=f"Failed to extract metadata from {corrupted_file.resolve()}"):
...
def test_subprocess_cli_nonzero_exit_code():
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        result = subprocess.run(
            ["python", "-c", "import sys; sys.exit(1)"],  # Simulate CLI failure
            capture_output=True
        )
...
def test_memory_overflow_protection():
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        with patch.object(FilesystemIndexer, 'populate_database', side_effect=MemoryError("Insufficient memory")):
            indexer = FilesystemIndexer.from_config_dict(config_dict, engine)
