...
def create_synthetic_hdf5_files(
    temp_dir: Path, num_files: int = 3, syllables_per_file: int = 5
) -> list[Path]:
    """
    ...
    for file_idx in range(num_files):
        ...
        with h5py.File(file_path, "w") as h5_file:
            ...
            audio_filenames = [
                f"audio_{file_idx:04d}_{i:03d}.wav" for i in range(syllables_per_file)
            ]
            h5_file.create_dataset(
                "audio_filenames", data=np.array(audio_filenames, dtype=h5py.string_dtype())
            )
...
def validate_ingestion_results(
    engine: Any, expected_files: list[Path], expected_syllables: int
) -> dict[str, Any]:
    """
...
class TestIngestHappyPath:
    ...
    def test_filesystem_scanning_discovery(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir_str:
            ...
            create_synthetic_hdf5_files(temp_dir, num_files=3)
            engine = setup_test_database(temp_dir)
    ...
    def test_deterministic_result_ordering(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir_str:
            ...
            create_synthetic_hdf5_files(temp_dir, num_files=3, syllables_per_file=2)
            engine = setup_test_database(temp_dir)
            ...
            # First run
            indexer.run_full_indexing(temp_dir)
            ...
            with get_session(engine) as session:
                syllables = (
                    session.execute(select(Syllable).order_by(Syllable.start_time, Syllable.id))
                    .scalars()
                    .all()
                )
    ...
    @pytest.mark.parametrize("num_files,syllables_per_file", [(1, 5), (3, 3), (5, 2)])
    def test_bulk_insertion_performance(self, num_files: int, syllables_per_file: int) -> None:
        ...
        with TemporaryDirectory() as temp_dir_str:
            ...
            created_files = create_synthetic_hdf5_files(
                temp_dir, num_files=num_files, syllables_per_file=syllables_per_file
            )
            engine = setup_test_database(temp_dir)
