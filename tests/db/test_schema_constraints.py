...
def test_foreign_key_cascade_delete_syllable_to_embedding(sqlite_engine: Engine) -> None:
    ...
    from ava.db.schema import Embedding, Recording, Syllable
    from ava.db.session import get_session
...
def test_foreign_key_cascade_delete_syllable_to_annotation(sqlite_engine: Engine) -> None:
    ...
    from ava.db.schema import Annotation, Recording, Syllable
    from ava.db.session import get_session
    ...
    with get_session(sqlite_engine) as session:
        ...
        annotation = Annotation(
            syllable_id=syllable.id, annotation_type="label", key="species", value="mouse"
        )
...
def test_referential_integrity_orphaned_syllable_prevented(sqlite_engine: Engine) -> None:
    ...
    with pytest.raises(IntegrityError):
        with get_session(sqlite_engine) as session:
            ...
            orphaned_syllable = Syllable(
                recording_id=999, spectrogram_path="/test/spec.h5", start_time=0.1, end_time=0.5
            )
...
def test_referential_integrity_orphaned_annotation_prevented(sqlite_engine: Engine) -> None:
    ...
    with pytest.raises(IntegrityError):
        with get_session(sqlite_engine) as session:
            ...
            orphaned_annotation = Annotation(
                syllable_id=999, annotation_type="label", key="species", value="mouse"
            )
...
def test_repository_delete_cascade_behavior(recording_repo: RecordingRepository) -> None:
    ...
    syllable = Syllable(
        recording_id=recording.id, spectrogram_path="/test/spec.h5", start_time=0.1, end_time=0.5
    )
...
def test_now_and_isoformat_datetime_usage(sqlite_engine: Engine) -> None:
    ...
    with get_session(sqlite_engine) as session:
        recording = Recording(
            file_path="/test/datetime.wav", checksum_sha256="time123", created_at=current_time
        )
        session.add(recording)
        session.flush()
