...
def main() -> None:
    ...
    parser.add_argument(
        "--allow-missing-roi",
        action="store_true",
        default=_env_flag("AVA_ALLOW_MISSING_ROI", False),
        help=(
            "Treat missing per-clip ROI records as skipped clips instead of failing "
            "the training job. Missing ROI directories and ROI parse errors remain fatal."
        ),
    )
    parser.add_argument(
            ...
            json.dumps(
                {
                    ...
                    "allow_missing_roi": bool(args.allow_missing_roi),
                },
                indent=2,
            )
        )
    ...
    try:
        ...
        coverage_proc = _stream_to_log(coverage_cmd, local_coverage_log_path, check=False)
        if coverage_proc.returncode != 0 and not (local_coverage_dir / "summary.json").exists():
            raise RuntimeError(
                f"Coverage command failed before writing a summary (exit={coverage_proc.returncode})."
            )
        ...
        summary["coverage_returncode"] = int(coverage_proc.returncode)
        summary["coverage_allow_missing_roi"] = bool(args.allow_missing_roi)
        ...
        if (
            ...
            or (
                int(coverage_summary.get("missing_roi_files") or 0)
                and not bool(args.allow_missing_roi)
            )
        ):
